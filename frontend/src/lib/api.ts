...
import type { Company, Fund, FundPayload, Receivable, ReceivablePayload } from './types';
...
export const api = {
  ...
  updateFund: (id: number, payload: FundPayload) => request<Fund>('/api/fundos/' + id, { method: 'PUT', body: JSON.stringify(payload) }),
  deleteFund: (id: number) => request<void>('/api/fundos/' + id, { method: 'DELETE' }),
  receivables: () => request<Receivable[]>('/api/recebiveis'),
  createReceivable: (payload: ReceivablePayload) => request<Receivable>('/api/recebiveis', { method: 'POST', body: JSON.stringify(payload) }),
  updateReceivable: (id: number, payload: ReceivablePayload) => request<Receivable>('/api/recebiveis/' + id, { method: 'PUT', body: JSON.stringify(payload) }),
