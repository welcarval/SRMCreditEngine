<script lang="ts">
  ...
  onMount(load);
  async function load() { try { funds = await api.funds(); } catch (err: unknown) { error = err instanceof Error ? err.message : 'Não foi possível carregar os fundos.'; } }
  async function remove(id: number) { if (!confirm('Excluir este fundo?')) return; try { await api.deleteFund(id); funds = funds.filter((fund) => fund.id !== id); notice = 'Fundo removido.'; } catch (err: unknown) { error = err instanceof Error ? err.message : 'Não foi possível excluir o fundo.'; } }
</script>
...
{#if selected !== undefined}<FundModal fund={selected} onclose={() => selected = undefined} onsaved={(saved: Fund) => { funds = selected ? funds.map((fund) => fund.id === saved.id ? saved : fund) : [...funds, saved]; selected = undefined; notice = 'Fundo salvo com sucesso.'; }} />{/if}
